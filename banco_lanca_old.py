...
def delete_dados_usina(nome_usina,col,cond):
    cursor.execute("DELETE FROM usina_{} WHERE {}='{}';".format(nome_usina,col,cond))
...
def check_null(nome_tabela,coluna_nome1,coluna_nome2, dado2,coluna_nome3, dado3,coluna_nome4, dado4):
     return cursor.execute(f"SELECT EXISTS(SELECT * FROM {nome_tabela} WHERE {coluna_nome1} IS NULL AND {coluna_nome2} = '{dado2}'\
        AND {coluna_nome3} = '{dado3}' AND {coluna_nome4} = '{dado4}')").fetchone()[0]
...
if __name__ == "__main__":
    ...
    for i in range(len(extrair_dados('dados'))):
        if extrair_dados('dados')[i][0] == 'AMERICA DO SUL' and extrair_dados('dados')[i][1] == 'BRASIL' and extrair_dados('dados')[i][2] == 'Usiminas'\
        and extrair_dados('dados')[i][3] == 'Ipatinga1':
            lista.append(extrair_dados('dados')[i][8:14])

    #df = pd.read_sql_query("SELECT * FROM dados", banco)
    #print(df.head())
    import os
    path = os.path.dirname(os.path.abspath(__file__))
    preencher_diametro("dados",'1','500','Ipatinga1','BRASIL','47','47','47','47','47','47','400','57')
