...
def parse_args(argv=None):
    ...
    parser.add_argument('--usi', type=str)
    parser.add_argument('--vida', type=str)
    ...
    parser.set_defaults(no_bar=False, display=False, resume=False, output_coco_json=False, output_web_json=False, shuffle=False,
                        ...
                        trained_model='weights/yolact_resnet_lanca_14181_113448_interrupt.pth', score_threshold=0.15, top_k=7, video_multiframe=1, display_masks=True, fast_nms=True)
#yolact_resnet_lanca_5922_112524_interrupt.pth
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.8, fps_str=''):
    """
...
############################################################################################################
    ...
    return img_numpy, centroFuros, diamArrayFuro, mascara
...
#############################################################################################################################################
def evalimage(pathimg, net:Yolact, path:str, save_path:str=None):
    ...
    if preds == [None]:
        return 0, 0, 0
    img_numpy, centroFuros, diametr, mascara = prep_display(preds, frame, None, None, undo_transform=False, class_color=True)
    if save_path is None:
        img_numpy = img_numpy[:, :, (2, 1, 0)]


    return img_numpy, centroFuros, diametr, mascara
#######################################################################################################################################
...
def evalimages(net:Yolact, input_folder:str, output_folder:str):
    ...
    for p in Path(input_folder).glob('*'):
        ...
        img_numpy, centroFuros, diametr, mascara=evalimage(net, path, out_path)
        print(path + ' -> ' + out_path)
    print('Done.')
...
def evaluate(pathimg, net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        if ':' in args.image:
            ...
            img_numpy, centroFuros , diametr, mascara= evalimage(pathimg, net, inp, out)
        else:
           img_numpy, centroFuros, diametr, mascara = evalimage(pathimg, net, args.image)
        return img_numpy, centroFuros, diametr, mascara
    elif args.images is not None:
        inp, out = args.images.split(':')
        img_numpy, centroFuros, diametr, mascara = evalimages(net, pathimg, out)
        return img_numpy, centroFuros, diametr
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                img_numpy, centroFuros, diametr, mascara = prep_display(preds, img, h, w)
            elif args.benchmark:
