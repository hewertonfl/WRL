...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model',
                        default='weights/yolact_lanca_82_246_interrupt.pth', type=str,
    ...
    parser.add_argument('--display_masks', default=False, type=str2bool,
    ...
    parser.add_argument('--image', default="1", type=str,
    ...
    parser.set_defaults(no_bar=False, display=False, resume=False, output_coco_json=False, output_web_json=False, shuffle=False,
                        benchmark=False, no_sort=False, no_hash=False, mask_proto_debug=False, crop=True, detect=False, display_fps=False, emulate_playback=False,
                        trained_model='weights/yolact_resnet_lanca_5922_112524_interrupt.pth', score_threshold=0.15, top_k=12, video_multiframe=1, display_masks=True, fast_nms=True)
#weights/yolact_resnet_lanca_1439_10075_interrupt.pth
...
def prep_display(dets_out, img, h, w, dist, undo_transform=True, class_color=False, mask_alpha=0.8, fps_str=''):
    """
    ...
    #img_numpy = img_numpy[:,:,::-1]                                 #INVERTE AZUL COM VERMELHO - LERDO 
    img_numpy = cv2.cvtColor(img_numpy, cv2.COLOR_BGR2RGB)           #INVERTE AZUL COM VERMELHO
    #img_numpy = np.flip(img_numpy, axis=2)                         #INVERTE AZUL COM VERMELHO - LERDO
    if args.display_fps:
    ...
    if args.display_text or args.display_bboxes:
        ...
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_text:
                    ...
                    font_thickness = 1
                    labeltam = "\n"
                    diamBox = ((x2-x1)+(y2-y1))/2
                    diamBox = (0.0009*int(dist)+0.0568)*diamBox
                    
                    areaPixel=np.count_nonzero(mascara[j, :])
                    diam=2*(np.sqrt(areaPixel/3.141592653589793))
                    diam = diam*0.4391
                    text_str2 = '%.3f' % diam
                    text_str = '%.3f' % diamBox
                    ladomm = "{:.3f}{}".format(diam, labeltam)
                    file = open('arq01.csv','a')
                    file.write(ladomm)
                    file.close()
                    
                    diamArray.append(diam)

                    text_w, text_h = cv2.getTextSize(text_str2, font_face, font_scale, font_thickness)[0]

                    text_pt = (x1, y1 - 3)
                    text_pt2 = (x1+80, y1-3)
                    text_color = [255, 255, 255]
                    #cv2.rectangle(img_numpy, (x1, y1), (x1 + text_w, y1 - text_h - 4), color, -1)##############################################################################
                    #cv2.putText(img_numpy, text_str2, text_pt, font_face, font_scale, text_color, font_thickness, cv2.LINE_AA)
                    #cv2.putText(img_numpy, text_str, text_pt2, font_face, font_scale, text_color, font_thickness, cv2.LINE_AA)
                    #######################################
                    if classes[j] == 1:
                        centroFuro=(int((x1+x2)/2), int((y1+y2)/2))
                        centroFuros.append(centroFuro)
                        boxesFuro.append(boxes[j])
                        diamArrayFuro.append(diam)
                    else:
                        boxExt = boxes[j]
                        diamExt = diam
                   
    
        shapeImg = img_numpy.shape
        ...
        for passnum in range(len(centroFuros)-1,0,-1):
                for i in range(passnum):
                    if angles[i]>angles[i+1]:
                        temp = angles[i]
                        temp2, temp3, temp4 = boxesFuro[i], diamArrayFuro[i], centroFuros[i]
                        angles[i] = angles[i+1]
                        boxesFuro[i], diamArrayFuro[i], centroFuros[i] = boxesFuro[i+1], diamArrayFuro[i+1], centroFuros[i+1]
                        angles[i+1] = temp
                        boxesFuro[i+1], diamArrayFuro[i+1], centroFuros[i+1] = temp2, temp3, temp4
                        
        for i in range(len(centroFuros)):
            text_str3 = '%d' % i
            text_pt3 = centroFuros[i]
            cv2.putText(img_numpy, text_str3, text_pt3, font_face, font_scale, text_color, font_thickness, cv2.LINE_AA)
            
        boxesFuro.append(boxExt)
        diamArrayFuro.append(diamExt)
        
    return img_numpy, boxesFuro, diamArrayFuro
...
#############################################################################################################################################
def evalimage(pathimg, net:Yolact, path:str, dist, save_path:str=None):

    #frame = torch.from_numpy(cv2.imread(pathimg)).cuda().float()
    frame = torch.from_numpy(cv2.imread(pathimg)).float()
    ...
    if preds == [None]:
        return 0, 0, 0
    img_numpy, boxes, diametr = prep_display(preds, frame, None, None, dist, undo_transform=False, class_color=True)
    if save_path is None:
        img_numpy = img_numpy[:, :, (2, 1, 0)]

    
    return img_numpy, boxes, diametr
#######################################################################################################################################




def evalimages(net:Yolact, input_folder:str, output_folder:str, dist):
    ...
    for p in Path(input_folder).glob('*'): 
        ...
        img_numpy=evalimage(net, path, out_path)
        print(path + ' -> ' + out_path)
    print('Done.')
...
def evaluate(pathimg, net:Yolact, dataset, dist, train_mode=False):
    ...
    if args.image is not None:
        if ':' in args.image:
            ...
            img_numpy, boxes , diametr= evalimage(pathimg, net, inp, out, dist)
        else:
           img_numpy, boxes, diametr = evalimage(pathimg, net, args.image, dist)
        return img_numpy, boxes, diametr
    elif args.images is not None:
        inp, out = args.images.split(':')
        img_numpy, boxes = evalimages(net, pathimg, out, dist)
        return img_numpy, boxes, diametr
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                img_numpy, boxes = prep_display(preds, img, h, w, dist)
            elif args.benchmark:
