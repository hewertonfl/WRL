...
def detect(imgToSeg, dist):
    with torch.no_grad():
        ...
        img_numpy, boxes, diametroFuros = eval2.evaluate(str, net, dataset, dist) ##Recebe a imagem segmentada e as boxes
        ...
        output = cv2.resize(output, dim, interpolation = cv2.INTER_AREA)      #Resize image to fit in gui
    ...
    return (output, boxes, diametroFuros)
