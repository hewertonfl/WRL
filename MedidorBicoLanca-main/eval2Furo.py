...
def parse_args(argv=None):
    ...
    parser.set_defaults(no_bar=False, display=False, resume=False, output_coco_json=False, output_web_json=False, shuffle=False,
                        ...
                        trained_model='weights/yolact_resnet_lanca_5922_112524_interrupt.pth', score_threshold=0.1, top_k=7, video_multiframe=1, display_masks=False, fast_nms=False)
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.6, fps_str=''):
    """
    ...
    return img_numpy, boxes, centros
...
def evalimage(pathimg, net:Yolact, path:str, save_path:str=None):
    ...
    if preds == [None]:
        return 0, 0, 0
    img_numpy, boxes, diamet = prep_display(preds, frame, None, None, undo_transform=False, class_color=True)
    if save_path is None:
        img_numpy = img_numpy[:, :, (2, 1, 0)]

    return img_numpy, boxes, diamet
#######################################################################################################################################
...
def evaluate(pathimg0, pathimg1, pathimg2, pathimg3, pathimg4, net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        if ':' in args.image:
            ...
            img_numpy[0], boxes[0] , diametr[0]= evalimage(pathimg0, net, inp, out)
            #img_numpy[1], boxes[1] , diametr[1]= evalimage(pathimg1, net, inp, out)
            #img_numpy[2], boxes[2] , diametr[2]= evalimage(pathimg2, net, inp, out)
            #img_numpy[3], boxes[3] , diametr[3]= evalimage(pathimg3, net, inp, out)
            #img_numpy[4], boxes[4] , diametr[4]= evalimage(pathimg4, net, inp, out)

        else:
            img_numpy[0], boxes[0] , diametr[0]= evalimage(pathimg0, net, args.image)
            #img_numpy[1], boxes[1] , diametr[1]= evalimage(pathimg1, net, args.image)
            #img_numpy[2], boxes[2] , diametr[2]= evalimage(pathimg2, net, args.image)
            #img_numpy[3], boxes[3] , diametr[3]= evalimage(pathimg3, net, args.image)
            #img_numpy[4], boxes[4] , diametr[4]= evalimage(pathimg4, net, args.image)
        return img_numpy, boxes, diametr
    elif args.images is not None:
        inp, out = args.images.split(':')
        img_numpy, boxes = evalimages(net, pathimg, out)
        return img_numpy, boxes, diametr
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                img_numpy, boxes = prep_display(preds, img, h, w)
            elif args.benchmark:
