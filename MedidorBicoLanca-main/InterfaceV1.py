...
def detectframe(imgToSeg, distancia_sensor):
    ...
    if detectAux:
                        ...
                        imgSeg, boxes, diamFuros = detectToInterface.detect(imgToSeg, distancia_sensor)
                        ...
                        if not isinstance(boxes, int):
                            ...
                            frame01 = cv2.cvtColor(imgToSeg, cv2.COLOR_BGR2RGB)
                            ...
                            if(len(diamFuros)>=7):
                                ...
                                for i in range(len(diamFuros)-1):
                                    if(diamFuros[i] > diamFuros[i+1]):
                                        diamFuros[i], diamFuros[i+1] = diamFuros[i+1], diamFuros[i]
                                ...
                                #Save image Original
                                cv2.imwrite(folder+"/"+"OriginalImage"+filename, imgToSeg)
                                #Save txt
