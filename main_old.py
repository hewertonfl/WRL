...
class Handler():
	...
	def __init__(self):
		...
		self.builder.add_from_file('./n_interface.glade')
		...
		self.mensagem_bico_nc1 = self.builder.get_object("mensagem_bico_nc1")
		...
		if check_caixa_vazia(self.reg_text) or check_caixa_vazia(self.pais_text) \
			...
			for i in range(len(dados_coluna('Grupo'))):
				...
				if dados_coluna('Grupo')[i] != self.grupos_text and dados_coluna('Site')[i] != self.site_text and dados_coluna('REGIAO')[i] != self.reg_text and dados_coluna('País')[i] != self.pais_text:
					...
					insert_usina(contar_linhas('Usinas'),self.reg_text,self.pais_text,self.grupos_text,self.site_text,self.bof_text,self.capacity_text,self.lancas_text,self.carros_text)
					usinas = dados_coluna('Grupo')
	...
	def on_button_sav2_clicked(self,widget):
		self.codigo_text = self.builder.get_object('entryb1').get_text()
		self.tipo_text = self.builder.get_object('entryb2').get_text()
		self.posicao_text = self.builder.get_object('entryb3').get_text()
		self.carro_text = self.builder.get_object('entryb4').get_text()
		self.conv_text = self.builder.get_object('entryb5').get_text()
		...
		elif verifica_existencia('dados','Bico_id',self.codigo_text,'grupo',guia,'site',site,'país',pais,'regiao',regiao)== 1:
	...
	def on_ver_bicos_clicked(self,widget):
		bico = "bico_usina_" + guia2
		if contar_linhas(bico) == 0:
			self.mensagem_bico_nc1.run()
		else:
	...
	def combobox_carro(self,grupo,site,nome_combobox):
		...
		carros = ler_colunas('Carro','dados','Grupo',grupo,'Site',site)
	...
	def on_button_start_clicked(self,widget):
		...
		operador = self.builder.get_object('entryc2').get_text()
		...
		if check_caixa_vazia(vida)  or check_caixa_vazia(operador) or check_caixa_vazia(data):
			self.mensagem_null.run()
				...
				process = subprocess.Popen(['python3', 'Interface1.py', '--cod', str(cod), '--usi', str(usin), '--vida', str(vida),
				'--site', str(site2),'--pais',str(pais2)], stdout=None, stderr=None)
				time.sleep(7)
				self.window.destroy()
			...
			if check_values("dados","vida",vida,'país',pais2,'grupo',usin,'site',site2) == 1:
				...
				if yes == True:
					...
					process = subprocess.Popen(['python3', 'Interface1.py', '--cod', str(cod), '--usi',
					str(usin), '--vida', str(vida),'--site', str(site2),'--pais',str(pais2)], stdout=None, stderr=None)
					time.sleep(7)
					self.window.destroy()
					...
					self.builder.get_object('entryc2').set_text('')
					self.builder.get_object('entryc3').set_text('')
					process = subprocess.Popen(['python3', 'Interface1.py', '--cod', str(cod), '--usi', str(usin),
					'--vida', str(vida),'--site', str(site2),'--pais',str(pais2)], stdout=None, stderr=None)
					time.sleep(7)
					self.window.destroy()
