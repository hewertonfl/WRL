...
def render_page_content(pathname):
    ...
    if pathname == "/":
        ...
        return html.Div(children=[
            ...
            html.Div(id='vida_images_container'),
        ])
...
def update_output_vida(value):
    ...
    if(str(filtered_dados.iloc[0]['Tipo']) == 'Slagless 4 Furos'):
        filtered_dados = filtered_dados[['Bico_id','Tipo','Vida','Carro','Operador','Data','Posição','D1','D2','D3','D4','D_Externo']] #filtered_dados[['Bico_id','Vida','Convertedor','Carro','Operador','Data','d1','d2','d3','d4']]
    else: #elif (str(filtered_dados.iloc[0]['Tipo']) == 'Slagless 6 Furos'):
        filtered_dados = filtered_dados[['Bico_id','Tipo','Vida','Carro','Operador','Data','Posição', 'D1','D2','D3','D4','D5','D6','D_Externo']] #filtered_dados[['Bico_id','Vida','Convertedor','Carro','Operador','Data', 'd1','d2','d3','d4','d5','d6']]
...
def update_image_vida(value):
    ...
    [
        ...
        dbc.CardImg(src="./assets/"+str(usinaToImage)+"-"+str(bicoToImage)+"-"+str(vidaToImage)+"-A.jpg", bottom=True),
    ],
    style={"width": "25rem"},
    )
    ...
    [
        ...
        dbc.CardImg(src="./assets/"+str(usinaToImage)+"-"+str(bicoToImage)+"-"+str(vidaToImage)+"-B.jpg", bottom=True),
    ],
    style={"width": "25rem"},
    )  
...
def update_graph(selected_bico):
    ...
    if(str(filtered_dados.iloc[0]['Tipo']) == 'Slagless 4 Furos'):
        line_fig = px.line(filtered_dados,
                           ...
                           range_y =(40,60),
                           title=f'Diâmetros do bico {selected_bico}')
        return line_fig
    else: #elif (str(filtered_dados.iloc[0]['Tipo']) == 'Slagless 6 Furos'):
        line_fig = px.line(filtered_dados,
